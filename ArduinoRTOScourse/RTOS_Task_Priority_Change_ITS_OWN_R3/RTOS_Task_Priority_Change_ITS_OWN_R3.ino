...
int blueTaskPrio; // zmienne do wyswietlania priorytetu
int redTaskPrio;
void setup()
{
  Serial.begin(9600);
  ...
  xTaskCreate(blueLEDController,"Blue Led",100,NULL,3,&blue_Handle); // zmieniam priorytet
...
void blueLEDController(void *pvParameters)
{
  ...
  {
   // BlueLEDProfiler++;
    //blueTaskPrio = uxTaskPriorityGet(blue_Handle);//UWAGA tutaj zobacze jaki mam priorytet dla niebieskiej
    blueTaskPrio = uxTaskPriorityGet(blue_Handle); // tak samo działa co linia wyżej
    redTaskPrio = uxTaskPriorityGet(red_Handle);
    Serial.print("Blue task priority: "); 
    Serial.println(blueTaskPrio); // bede wyswietlac jaki mam obecnie priorytet
